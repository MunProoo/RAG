...
문서에게 질문하면, 컴퓨터가 **관련 문서 조각을 찾아** 그 내용을 바탕으로 답해 주는 도우미입니다.  
공개 데모는 `rag/data/docs/sample_*.md` 기준이며, 실제 운영 문서는 로컬/비공개로 넣습니다.
...
### Swagger / FaceWT·출입그룹 API

API 질문은 User Guide나 프로토콜만으로 대체하면 안 되고, **`swagger_kr.md` 쪽**을 근거로 답해야 합니다.

- 「출입그룹 API」처럼 **한글**로 물어도 swagger 경로(`/v1/accessGroups` 등)가 나와야 합니다. `AccessGroup`처럼 영어를 정확히 써야만 답이 나오는 상태는 완화되어 있습니다.
- FaceWT는 사용자 `faceWTInfo`뿐 아니라 단말기 **`/scan/facewt`** 도 함께 나와야 합니다.
- 「출입그룹 어떻게 추가해?」처럼 **화면 조작**만 물으면 User Guide 절차를 따릅니다(API와 섞지 않음).

좋은 답의 기준 예시: `rag/data/eval/artifacts/ideal_answer_facewt_swagger_kr.md`, `rag/data/eval/doc_qa/swagger_kr/`

### User Guide (타임존·공휴일·모니터링)

- 타임존·공휴일·출입구역 등은 **가이드 메뉴 경로**로 답합니다.
- 단말기 **연결 상태**는 모니터링의 **녹색/빨간 원** 의미와 출입문 아이콘 설명이 포함되는 것이 좋습니다.
- 「일반설정에서 자동 동기화」는 설정 옵션을 켜는 절차가 중심이고, 수동으로 단말에 보내는 방법과 섞이면 안 됩니다.

질문·예상 답 모음: `rag/data/eval/doc_qa/alpeta_user_guide_v2/`

### 프로토콜 “전부 리스트”·위겐드

목차처럼 긴 목록은 중간만 잘리면 안 됩니다. “전부/전체/리스트업”이라고 물으면 **끝까지** 나와야 합니다.  
「위겐드/wiegand 설정」은 Set Wiegand(`0x0041`)와 설정 필드(`WiegandConfig`)가 빠져서는 안 됩니다.
...
3. 질문어를 문서에 나온 단어에 가깝게 바꿔 봅니다.  
   예: “자동빌드”, “FaceWT”, “출입그룹 API”, `[단말기리스트]`  
   (한글·영문 동의어는 많이 보강했지만, 문서에 있는 고유 명칭이 있으면 더 안정적입니다.)
...
- **회귀 테스트** (`test_rag_regression.py`): 약 **137개** 자동 검사 (AI 모델 없이 가능)
- **문서별 Q&A** (`rag/data/eval/doc_qa/`): 원본 가이드와 맞춰 둔 질문·예상 답
