...
> **공개 저장소 안내:** 사내 원본 문서·평가 덤프는 포함하지 않습니다. 데모는 `rag/data/docs/sample_*.md` 를 사용하세요.  
> 파이프라인 코드에는 과거 도메인용 규칙 문자열이 남아 있을 수 있으며, 완전 익명화는 별도 작업입니다.
...
“그거 자세히 설명해줘”처럼 질문만으로 대상을 알 수 없으면 최근 대화를 반영해 독립적인 질문으로 바꾼다. 「표로」「정리해줘」「표를 활용해서」「보기 쉽게」처럼 **재포맷만** 요청해도 동일하다.

```text
그거 자세히 설명해줘
→ Alpeta 프로토콜의 Param3을 자세히 설명해줘

(자동빌드 답변 후) 표를 활용해서 더 보기 쉽게 해줘
→ alpeta 자동빌드(자동화 버전) 절차를 표로 … 정리해줘
```

**규칙 문맥화·주제 가드**(LLM보다 우선하는 경우):
...
이 단계는 `CONTEXTUALIZE_FOLLOW_UP=true`이고 후속/재포맷으로 감지됐을 때 동작한다. 규칙은 즉시 적용하고, 해당 없으면 `REWRITE_MODEL` LLM 문맥화를 호출한다.

**MediaServer / Test.md 인물 / 자동빌드 표 후속**: 하드웨어 스펙 표는 `MediaServer_Specs_New.md`만 사용한다. `Test.md` 인물(복수 이름 포함)은 프로필 청크만 답하고 UG·카메라·VMS를 섞지 않는다. 자동빌드 「표로/보기 쉽게」후속은 NSIS 매뉴얼 자동화 절을 유지한다.
